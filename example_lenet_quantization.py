...
from admm import ADMM_quantization
...
def train(args, model, device, train_loader, optimizer, epoch, writer, admm):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss.backward()
        if epoch >= args.admm_update_start:
...
def main():
    ...
    parser.add_argument('--admm-bits', type=list, default=[4 for i in range(4)], metavar='N',
                        help='bits of quantization')
    ...
    logdir = './summaries/mnist_quantization_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    writer = SummaryWriter(logdir)

    admm = ADMM_quantization(model, update_interval=args.admm_update_interval, bits=args.admm_bits)
    ...
    for index, m in enumerate(model.modules()):
        if isinstance(m, (nn.Conv2d, nn.Linear)):
            print(m)
            unique, count = torch.unique(m.weight, sorted=True, return_counts= True)
            print(unique,":",count)
